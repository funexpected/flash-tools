...
interface FlashDocument {
    ...
    exportInstanceToLibrary(frameNumber:number, bitmapName:string);
...
interface FlashInstance extends FlashElement {
    instanceType?: string;
...
// if FlashElement.elementType == 'instance'
interface FlashBitmapInstance extends Element {
...
interface FlashCompiledClipInstance extends FlashElement {
    accName: string;
    actionScript: string;
    description: string;
    forceSimple: boolean;
...
interface FlashSymbolInstance extends FlashElement {
    accName: string;
    actionScript: string;
...
interface FlashComponentInstance extends FlashElement {
    parameters: any[];
}
 ...
 */
interface FlashShape extends FlashElement {
...
interface FlashSymbolItem extends FlashItem {
...
interface FlashFolderItem extends FlashItem {
}

interface FlashFontItem extends FlashItem {
...
interface FlashVideoItem extends FlashItem {
...
interface FlashOval extends FlashShape {
...
declare type SpriteSheetAlgorithm = "basic" | "maxRects";
declare type FlashImageFormat = "png" | "jpg";
declare type FlashBitDepth = 8 | 24 | 32;    

declare class SpriteSheetOptions {
    format: FlashImageFormat;
    backgroundColor?: any;
    quality?: number;
    bitDepth?: FlashBitDepth;
...
declare class SpriteSheetExporter {
  ...
  algorithm:SpriteSheetAlgorithm;
  allowRotate:boolean;
  ...
  changeSymbol();
  exportSpriteSheet(fileURL:string, format: FlashImageFormat|SpriteSheetOptions, writeMetaData?:boolean):string;
  format:string;
  image:string;
  layoutFormat:string;
  ...
  overflowed:boolean;
  removeBitmap(bitmap: FlashBitmapItem);
  removeSymbol();
  shapePadding:number;
  sheetHeight:number;
  sheetWidth:number;
